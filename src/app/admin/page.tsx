...
export default async function AdminPage({
  ...
  return (
    <div className="min-h-dvh bg-titik-kader">
      <Kepala user={user} warna="var(--teal-gelap)" />
      <main className="mx-auto max-w-3xl px-4 py-6">
        <h1 className="font-judul text-lg font-extrabold text-[var(--teal-tua)]">Kelola Akun</h1>
        ...
        <section className="pop mt-5 rounded-[var(--r-kartu)] border-2 border-[var(--garis-kader)] bg-[var(--kartu)] p-4">
          <div className="flex items-center justify-between gap-3">
            <div className="min-w-0">
              <h2 className="font-judul text-sm font-extrabold text-[var(--teal-tua)]">🔄 Sinkron SIMPUS</h2>
              ...
              {!cache && (
                <p className="mt-1 inline-block rounded-full px-2 py-0.5 text-[10px] font-bold" style={{ background: "var(--kuning-muda)", color: "var(--kuning-teks)" }}>
                  ⚠️ Perlu SIMPUS_DATABASE_URL &amp; SIMPUS_DEK di env server
                </p>
              )}
            </div>
        ...
        <section className="mt-8">
          <h2 className="font-judul text-sm font-extrabold">Kader ({kader.length})</h2>
          <div className="mt-2 space-y-2">
            ...
            {kader.map((u) => (
              <div
                key={u.id}
                className="rounded-xl border-2 border-[var(--garis-kader)] bg-[var(--kartu)] px-4 py-3"
              >
                <div className="flex items-center justify-between gap-3">
                  <div className="min-w-0">
                    <p className="truncate text-sm font-bold">
                      ...
                      {!u.aktif && (
                        <span className="ml-2 rounded bg-[var(--merah-muda)] px-1.5 py-0.5 text-[10px] font-bold text-[var(--merah-teks)]">
                          NONAKTIF
                        </span>
                      )}
                      {u.perluGantiSandi && (
                        <span className="ml-2 rounded px-1.5 py-0.5 text-[10px] font-bold" style={{ background: "var(--kuning-muda)", color: "var(--kuning-teks)" }}>
                          sandi sementara
                        </span>
                      )}
                    </p>
                  ...
                  <div className="flex shrink-0 items-center gap-2">
                    ...
                    <form action={setAktif}>
                      ...
                      <button className="btn-garis border-2 border-[var(--garis)] px-2.5 py-1.5 text-[11px] text-[var(--teks-sekunder)] hover:bg-[var(--bg)]">
        ...
        <section className="mt-8">
          <h2 className="font-judul text-sm font-extrabold">Orang tua ({ortu.length})</h2>
          <div className="mt-2 space-y-2">
            {ortu.length === 0 && (
            ...
            {ortu.map((u) => (
              <div
                key={u.id}
                className="flex items-center justify-between gap-3 rounded-xl border-2 border-[var(--garis-kader)] bg-[var(--kartu)] px-4 py-3"
              >
        ...
        <section className="mt-8">
          <h2 className="font-judul text-sm font-extrabold">Admin ({admin.length})</h2>
          <div className="mt-2 space-y-2">
            {admin.map((u) => (
              <div
                key={u.id}
                className="rounded-xl border-2 border-[var(--garis-kader)] bg-[var(--kartu)] px-4 py-3 text-sm"
              >
                <span className="font-bold">{u.nama}</span>{" "}
                <span className="text-[var(--teks-sekunder)]">@{u.username}</span>
              </div>
