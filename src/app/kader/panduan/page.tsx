/* Panduan kader — rangkuman jadwal & catatan NTB (diporting dari app KADER lama,
...
export default function Panduan() {
  return (
    <main className="mx-auto max-w-3xl px-4 py-5">
      <h1 className="text-lg font-extrabold text-[var(--teal-tua)]">Panduan Imunisasi</h1>
      ...
      <section className="mt-4 space-y-2">
        {JADWAL.map((j) => (
          <div key={j.usia} className="flex items-center gap-3 rounded-2xl border border-[var(--garis)] bg-[var(--kartu)] p-3">
            <img src={j.img} alt="" width={52} height={52} className="h-13 w-13 shrink-0 rounded-xl bg-[var(--bg)] object-contain" />
            <div>
              <p className="text-sm font-extrabold text-[var(--teal-tua)]">{j.usia}</p>
      ...
      <section className="mt-5 rounded-2xl border border-[var(--garis)] bg-[var(--kartu)] p-4 text-xs leading-relaxed">
        <p className="font-extrabold text-[var(--teal-tua)]">Catatan NTB (penting)</p>
        <ul className="mt-1.5 list-disc space-y-1 pl-4">
          ...
          <li><b>JE</b> (Japanese Encephalitis) tidak tersedia gratis di NTB — hanya klinik <span className="font-bold text-[var(--coral)]">SWASTA</span>, tidak masuk IDL/IBL.</li>
          <li><b>Hexavalen</b> (6-in-1) sudah mengandung IPV → tidak perlu IPV terpisah. <b>Rotarix</b> cukup 2 dosis; <b>Rotavac</b> 3 dosis.</li>
        </ul>
      </section>

      <h2 className="mt-6 text-sm font-extrabold">Edukasi untuk Orang Tua</h2>
      <section className="mt-2 grid gap-2 sm:grid-cols-2">
        {EDUKASI.map((e) => (
          <div key={e.judul} className="flex items-start gap-3 rounded-2xl border border-[var(--garis)] bg-[var(--kartu)] p-3">
            <img src={e.img} alt="" width={44} height={44} className="h-11 w-11 shrink-0 rounded-xl bg-[var(--bg)] object-contain" />
            <div>
