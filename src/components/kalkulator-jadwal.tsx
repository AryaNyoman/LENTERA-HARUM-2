...
/** Isi halaman kalkulator jadwal, dipakai kader & ortu (peran ubah aksen border/fokus
 *  input tanggal saja — warna kartu status terlewat/kuning/teal tetap sama). */
export default function KalkulatorJadwal({ peran }: { peran: "kader" | "ortu" }) {
  const [tgl, setTgl] = useState("");
  const borderInput = peran === "ortu" ? "border-[var(--coral-pastel)] focus:border-[var(--coral)]" : "border-[var(--teal-pastel)] focus:border-[var(--teal)]";
    ...
    <div className="mx-auto max-w-md px-4 pt-4">
      <label className="pop block text-[12.5px] font-extrabold text-[var(--teks-3)]">
        ...
        <InputTanggal
          ...
          className={`h-[52px] w-full rounded-2xl border-2 ${borderInput} bg-[var(--kartu)] px-4 text-base font-bold outline-none transition-colors`}
        />
      </label>
